...
CORS(app,
     resources={
         r"/api/*": {
             "origins": ["http://localhost:3000", "http://localhost:8010", "https://ahut.site:8081","https://ahut.site:8083", "http://ahut.site:8090", "http://ahut.site:9000", "http://ahut.site:8010", "http://ahut.site:9090"],
...
@app.route("/user/backstage", methods=['GET'])
def backstage():
    cookied_username = request.cookies.get('username')
    if cookied_username is not None and auth_manager.isAuthenticated(cookied_username):
...
def get_database_celebrity_detail(celebrity_id):
    ...
    data = cur.fetchone()
    keyList = ["id", "name", "gender", "zodiac", "living_time", "birthday", "left_day", "birthplace", "occupation",
               "is_director",
...
def get_database_tables():
    ...
    for tableName in cur.fetchall():
        if "_" not in tableName[0] and "recommendations" not in tableName[0]:
...
def get_database_data(tableName):
    ...
    pageSize = json_content['pageSize']
    totalPage = json_content['totalPage']

    print(f"按表各取数据 page: {page} pageSize: {pageSize} totalPage: {totalPage}")
    ...
    elif tableName == "celebrity":
        cur.execute("SELECT * FROM celebrity LIMIT 20")
    elif tableName == "recommendations":
        cur.execute("SELECT * FROM recommendations LIMIT 20")
    if tableName == "review":
        cur.execute("SELECT * FROM review LIMIT 20")
    elif tableName == "area":
        cur.execute("SELECT * FROM area LIMIT 20")
    elif tableName == "type":
        cur.execute("SELECT * FROM type LIMIT 20")
    elif tableName == "tag":
        cur.execute("SELECT * FROM tag LIMIT 20")
    if tableName == "language":
        cur.execute("SELECT * FROM language LIMIT 20")
    else:
        pass
